...
def _parse_timestamp_segment(segment, time_offset):
    ...
    try:
        if isinstance(segment, dict):
            ...
            print(
                f"⚠️ Warning: Unknown timestamp segment format: {type(segment)} - {repr(segment)}"
            )
            start = end = 0
...
def handler(job):
    ...
    if language is not None:
        if isinstance(language, str) and language.lower() == "auto":
            ...
            return {
                "error": f"Invalid language '{language}'. Must be 'auto' or one of: {', '.join(VALID_LANGUAGES)}"
            }
    ...
    try:
        ...
        for idx, chunk in enumerate(chunks_info):
            ...
            if chunk_duration < 0.5:
                print(
                    f"  ⏭️ Skipping chunk {idx + 1}/{len(chunks_info)} (too short: {chunk_duration:.2f}s)"
                )
                continue
            ...
            if use_previous_context and text:
                ...
                current_context = (
                    text.strip()[-200:] if len(text) > 200 else text.strip()
                )
            else:
        ...
        if "Download timeout" in error_str:
            user_message = (
                "Audio download timed out. Please check the URL and try again."
            )
            ...
            user_message = (
                "GPU processing error. The service may be temporarily overloaded."
            )
