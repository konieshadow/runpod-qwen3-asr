...
from utils import find_split_points, download_audio, MERGE_THRESHOLD
...
def test_split_logic():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(
            f"\n测试参数: max_chunk_ms={max_chunk_ms}, min_silence_ms={min_silence_ms}, merge_threshold={merge_threshold}"
        )

        split_points = find_split_points(
            audio, max_chunk_ms, min_silence_ms, merge_threshold
        )
        ...
        for i in range(len(split_points) - 1):
            ...
            print(
                f"片段 {i + 1}: {start_ms / 1000:.2f}s -> {end_ms / 1000:.2f}s (长度: {length_sec:.2f}s)"
            )
        ...
        long_segments = [
            ...
            if 60 <= l < max_chunk_ms / 1000 * merge_threshold
        ]
        very_long_segments = [
            l for l in segment_lengths if l >= max_chunk_ms / 1000 * merge_threshold
        ]
        ...
        print(
            f"  60-{max_chunk_ms / 1000 * merge_threshold:.0f}s: {len(long_segments)} 个"
        )
        print(
            f"  >= {max_chunk_ms / 1000 * merge_threshold:.0f}s: {len(very_long_segments)} 个"
        )
        ...
        max_merged_length = max_chunk_ms / 1000 * merge_threshold

        # 检查1: 任意分段 <= max_chunk_ms
        for i, l in enumerate(segment_lengths):
            if l > max_chunk_ms / 1000:
                print(
                    f"  ❌ 分段 {i + 1} 超过最大时长: {l:.2f}s > {max_chunk_ms / 1000}s"
                )
                all_valid = False

        # 检查2: 分段不应超过合并阈值（关键修复验证）
        for i, l in enumerate(segment_lengths):
            if l > max_merged_length:
                print(
                    f"  ⚠️  分段 {i + 1} 接近上限: {l:.2f}s > {max_merged_length:.2f}s (建议检查)"
                )

        # 检查3: 不存在连续短分段（连续总和 < max_chunk_ms * merge_threshold）
        for i in range(len(segment_lengths) - 1):
            combined = segment_lengths[i] + segment_lengths[i + 1]
            if combined < max_merged_length:
                print(
                    f"  ❌ 分段 {i + 1} 和 {i + 2} 连续总时长过短: {combined:.2f}s < {max_merged_length:.2f}s"
                )
                all_valid = False
